...
# quickly generate a file with all public and private IPs
aws-eips:
  #!/usr/bin/env nu
  ( tofu show -json
  | from json
      ...
      flake.nixosModules.ipmodule = {
        ...
        options = {
          publicIpv4 = lib.mkOption {
            type = lib.types.str;
            default = all.${name}.publicIpv4;
          };
          publicIpv6 = lib.mkOption {
            type = lib.types.str;
            default = all.${name}.publicIpv6;
          };
        };
      };
    }
  ...
  | str join "\n"
  | alejandra --quiet -
  | save --force flake/nixosModules/ips.nix
  )
  git add flake/nixosModules/ips.nix
