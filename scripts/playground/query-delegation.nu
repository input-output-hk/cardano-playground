...
def sops-decrypt [file: string] {
    ...
    debug-cmd "sops" ["--config", $config, "--input-type", "binary", "--output-type", "binary", "--decrypt", $file]
...
def is-debug [] {
    ("DEBUG" in $env) and ($env.DEBUG | is-not-empty)
}
...
def check-node-synced [environment: string, net_args: list<string>] {
    ...
    if $sync_pct < 100.0 {
        error make --unspanned { msg: $"Node is only ($sync_pct)% synced on ($environment). Wait for 100% before querying." }
    }
    $tip_json
}
...
def find-groups [environment: string] {
    ...
    ls $groups_dir | where type == dir
        | each { |d| $d.name | path basename }
        | where { |name| $name =~ $"^($environment)\\d+$" }
        | sort
}
...
def query-pool-stake [pool_id: string, net_args: list<string>] {
    ...
    let pool_data = ($data | get pools | transpose _hash info | first | get info)
    {
...
def query-drep-info [vkey_path: string, net_args: list<string>] {
    ...
    try {
        ...
        debug-cmd "cardano-cli" ["latest", "governance", "drep", "id", "--drep-verification-key-file", $f_vkey]
        let drep_id = (^cardano-cli latest governance drep id
            --drep-verification-key-file $f_vkey | str trim)
        debug-cmd "cardano-cli" (["latest", "query", "drep-state", "--drep-verification-key-file", $f_vkey, "--include-stake"] | append $net_args)
        let state = (^cardano-cli latest query drep-state
            --drep-verification-key-file $f_vkey --include-stake ...$net_args | from json)
        rm --force $f_vkey

        if ($state | is-empty) {
            return { drep_id: $drep_id, stake: 0, expiry: null }
        }
        ...
        rm --force $f_vkey
        error make --unspanned { msg: $"Failed to query DRep: ($e.msg)" }
    }
}
...
def query-drep-totals [epoch: int, net_args: list<string>] {
    ...
    let always_abstain  = ($dist_entries | where key == "drep-alwaysAbstain"      | get -o value | safe-sum)
    let always_noconf   = ($dist_entries | where key == "drep-alwaysNoConfidence" | get -o value | safe-sum)
    ...
    let active_stake = ($drep_state
        | where { |e| ($e | get 1 | get -o expiry | default 0) >= $epoch }
        | each { |e| $e | get 1 | get -o stake | default 0 }
        | safe-sum)

    let expired_stake = ($drep_state
        | where { |e| ($e | get 1 | get -o expiry | default 0) < $epoch }
        | each { |e| $e | get 1 | get -o stake | default 0 }
        | safe-sum)

    let active_count = ($drep_state
        | where { |e| ($e | get 1 | get -o expiry | default 0) >= $epoch }
        | length)

    let expired_count = ($drep_state
        | where { |e| ($e | get 1 | get -o expiry | default 0) < $epoch }
        | length)
...
def query-pool-totals [net_args: list<string>] {
    ...
    let abstain_total  = ($pool_dist | where { |e| ($e | get -o 2) == "drep-alwaysAbstain" }      | each { |e| $e | get 1 } | safe-sum)
    let noconf_total   = ($pool_dist | where { |e| ($e | get -o 2) == "drep-alwaysNoConfidence" } | each { |e| $e | get 1 } | safe-sum)
...
def safe-sum [] {
    if ($in | is-empty) { 0 } else { $in | math sum }
}
...
def threshold-pct [val] {
    let pct_val = if ($val | describe | str starts-with 'record') {
        ...
        if $den != 0 { ($num / $den * 100) | math round --precision 2 } else { 0.0 }
    } else {
...
def main [
    ...
    let drep_totals = (try {
        ...
        { stake_total: 0, active_stake: 0, expired_stake: 0, always_abstain: 0, always_noconf: 0, active_count: 0, expired_count: 0 }
    })
    ...
    let pool_totals = (try {
        ...
        { stake_total: 0, abstain_total: 0, noconf_total: 0, pool_count: 0 }
    })
    print "                                        "
    ...
    let drep_summary = [
        { Metric: "DRep Stake Total",       Value: (lovelace-to-ada $drep_totals.stake_total),    Detail: "all dreps + system" }
        { Metric: "DRep Active Stake",       Value: (lovelace-to-ada $drep_totals.active_stake),   Detail: $"($drep_totals.active_count) drep\(s\)" }
        { Metric: "DRep Expired Stake",      Value: (lovelace-to-ada $drep_totals.expired_stake),  Detail: $"($drep_totals.expired_count) drep\(s\) — excluded from voting" }
        { Metric: "Always Abstain",          Value: (lovelace-to-ada $drep_totals.always_abstain), Detail: "excluded from denominator" }
        { Metric: "Always No Confidence",    Value: (lovelace-to-ada $drep_totals.always_noconf),  Detail: "in denominator; votes Yes on NoConfidence" }
        { Metric: "DRep Vote Denominator",   Value: (lovelace-to-ada $drep_denominator),           Detail: "active + noConfidence" }
        { Metric: "Our DRep Stake",          Value: (lovelace-to-ada $our_drep_stake),             Detail: (pct $our_drep_stake $drep_denominator) }
    ]
    ...
    let pool_summary = [
        { Metric: "Pool Stake Total",        Value: (lovelace-to-ada $pool_totals.stake_total),   Detail: $"($pool_totals.pool_count) pool\(s\)" }
        { Metric: "Pool Abstain Total",      Value: (lovelace-to-ada $pool_totals.abstain_total), Detail: "SPOs delegated to alwaysAbstain" }
        { Metric: "Pool No Confidence Total", Value: (lovelace-to-ada $pool_totals.noconf_total),  Detail: "SPOs delegated to alwaysNoConfidence" }
        { Metric: "Pool Vote Denominator",   Value: (lovelace-to-ada $pool_denominator),          Detail: "total - abstain (varies by action type)" }
        { Metric: "Our Pools Stake (mark)",  Value: (lovelace-to-ada $our_pool_mark_total),       Detail: (pct $our_pool_mark_total $pool_denominator) }
    ]
    ...
    let cc_threshold_raw = ($gov_state | get -o committee | get -o threshold | default 0)
    ...
    let thresholds = [
        { "Action Type": "No Confidence",        "DRep (%)": (threshold-pct ($dvt | get committeeNoConfidence)), "Pool (%)": (threshold-pct ($pvt | get committeeNoConfidence)), "CC (%)": "—" }
        { "Action Type": "Update Committee",      "DRep (%)": (threshold-pct ($dvt | get committeeNormal)),       "Pool (%)": (threshold-pct ($pvt | get committeeNormal)),       "CC (%)": "—" }
        { "Action Type": "New Constitution",      "DRep (%)": (threshold-pct ($dvt | get updateToConstitution)),  "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "Hard Fork",             "DRep (%)": (threshold-pct ($dvt | get hardForkInitiation)),    "Pool (%)": (threshold-pct ($pvt | get hardForkInitiation)),    "CC (%)": $cc_threshold }
        { "Action Type": "PP Network Group",      "DRep (%)": (threshold-pct ($dvt | get ppNetworkGroup)),        "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "PP Economic Group",     "DRep (%)": (threshold-pct ($dvt | get ppEconomicGroup)),       "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "PP Technical Group",    "DRep (%)": (threshold-pct ($dvt | get ppTechnicalGroup)),      "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "PP Governance Group",   "DRep (%)": (threshold-pct ($dvt | get ppGovGroup)),            "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "PP Security Group",     "DRep (%)": "max of applicable groups",                         "Pool (%)": (threshold-pct ($pvt | get ppSecurityGroup)),       "CC (%)": $cc_threshold }
        { "Action Type": "Treasury Withdrawal",   "DRep (%)": (threshold-pct ($dvt | get treasuryWithdrawal)),    "Pool (%)": "—",                                                "CC (%)": $cc_threshold }
        { "Action Type": "Info Action",           "DRep (%)": "n/a",                                              "Pool (%)": "n/a",                                              "CC (%)": "n/a" }
    ]
