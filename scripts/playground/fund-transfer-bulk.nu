...
def parse-lovelace [amount: string] {
  let lovelace = try {
    ...
    error make --unspanned {
      msg: $"Invalid lovelace amount: '($amount)' — must be a whole number"
    }
  }
  if $lovelace <= 0 {
    error make --unspanned {
      msg: $"Amount must be positive, got: ($amount) lovelace"
    }
  }
  $lovelace
}
...
def parse-transfers-file [path: string] {
  if not ($path | path exists) {
    error make --unspanned {
      msg: $"Transfers file not found: ($path)"
    }
  }
  ...
  if ($transfers | is-empty) {
    error make --unspanned {
      msg: $"No transfers found in ($path)"
    }
  }
  $transfers
}
...
def main [
  ...
  if $sync_pct < 100.0 {
    error make --unspanned {
      msg: $"Node is only ($sync_pct)% synced. Wait for it to reach 100% before transferring funds."
    }
  }
  ...
  let available = if ($lovelace_utxos | is-empty) { 0 } else {
  ...
  if $total_lovelace > $available {
    rm --force $f_skey
    error make --unspanned {
      msg: $"Insufficient funds: need (lovelace-to-ada $total_lovelace) but only (lovelace-to-ada $available) available in lovelace-only UTxOs."
    }
  }
  if $dry_run {
  ...
  for utxo in $lovelace_utxos { $build_args = ($build_args | append [
    "--tx-in"
    $utxo.txin
  ]) }
  for transfer in $transfers { $build_args = ($build_args | append [
    "--tx-out"
    $"($transfer.address)+($transfer.lovelace)"
  ]) }
