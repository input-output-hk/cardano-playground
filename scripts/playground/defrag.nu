...
def main [
    ...
    if $sync_pct < 100.0 {
        error make --unspanned { msg: $"Node is only ($sync_pct)% synced. Wait for it to reach 100% before defragmenting." }
    }
    ...
    let total_lovelace = if ($lovelace_utxos | is-empty) { 0 } else { $lovelace_utxos | each { |u| $u.lovelace } | math sum }

    # Native-token-bearing UTxOs
    let token_utxos = ($all_utxos
        | where { |row| ($row.value.value | columns | length) > 1 }
        | each  { |row| { txin: $row.key, lovelace: $row.value.value.lovelace } })
    let token_count = ($token_utxos | length)
    let total_token_lovelace = if ($token_utxos | is-empty) { 0 } else { $token_utxos | each { |u| $u.lovelace } | math sum }
