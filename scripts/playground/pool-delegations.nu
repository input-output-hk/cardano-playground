...
# ─── Paths ────────────────────────────────────────────────────────────────────
def secrets-root [environment: string] { $"secrets/envs/($environment)/pool-delegations" }
def mnemonic-path [environment: string] { $"(secrets-root $environment)/pool-delegations.mnemonic" }
def account-dir [environment: string, index: int] { $"(secrets-root $environment)/($index)" }
...
def check-account-exists [environment: string, index: int] {
  ...
  if $n == 0 {
    error make --unspanned {
      msg: $"No accounts generated yet for ($environment). Run 'generate-accounts' first."
    }
  }
  if $index >= $n {
    error make --unspanned {
      msg: $"Account index ($index) has not been generated. Only ($n) account\(s\) exist \(indices 0..($n - 1)\). Run 'generate-accounts -n ($index + 1)' to create it."
    }
  }
}
...
def sops-encrypt [content: string, file: string] {
  ...
  try {
    ...
    rm --force $file
    error make --unspanned {
      msg: $"Failed to sops-encrypt ($file). Check .sops.yaml creation rules."
    }
  }
}
# ─── Display helpers ──────────────────────────────────────────────────────────
...
def check-node-synced [environment: string, net_args: list<string>] {
  ...
  if $sync_pct < 100.0 {
    error make --unspanned {
      msg: $"Node is only ($sync_pct)% synced. Wait for it to reach 100% before proceeding."
    }
  }
  print ""
}
...
def query-balance [address: string, net_args: list<string>] {
  ...
  $data | transpose key value | each { |row| $row.value.value.lovelace? | default 0 } | math sum
}
# Returns the largest lovelace-only UTxO as {txin, lovelace}
def largest-lovelace-utxo [address: string, net_args: list<string>] {
  let data = (^cardano-cli latest query utxo --address $address --output-json ...$net_args | from json)
  let utxos = (
    $data | transpose key value | where { |row| ($row.value.value | columns | length) == 1 } | each { |row| { txin: $row.key, lovelace: $row.value.value.lovelace } } | sort-by lovelace --reverse
  )
  if ($utxos | is-empty) {
    error make --unspanned {
      msg: $"No lovelace-only UTxO found at ($address)"
    }
  }
  $utxos | first
}
...
def query-pool-info [pool_id: string, net_args: list<string>] {
  ...
  let margin = if $margin_raw != null {
    if ($margin_raw | describe | str starts-with 'record') {
      ...
      if $den != 0 {
        (($num | into float) / $den * 100) | math round --precision 2
      } else { null }
    } else {
  ...
  let meta_url = if $meta_section != null {
    $meta_section | get -o url
  } else { null }
  let meta = if $meta_url != null {
    try {
      ^curl -fkLs $meta_url | from json
    } catch { null }
  } else { null }
...
def tx-delegate [
  ...
  let build_args = (
    [
      ...
      $utxo.txin
      "--change-address"
      $acct.delegation_address
      "--witness-override"
      "2"
    ] | append $cert_args | append ["--out-file", $f_tx_body] | append $net_args
  )
...
def tx-dedelegate [acct: record, net_args: list<string>, skip_confirmation: bool] {
  ...
  if $stake_info.rewards > 0 {
    $tx_args = ($tx_args | append [
      "--withdrawal"
      $"($acct.stake_address)+($stake_info.rewards)"
    ])
  }
...
def tx-defund [
  ...
  for txin in $txins { $build_args = ($build_args | append ["--tx-in", $txin]) }
...
def do-status [environment: string, num_accounts: int, dbsync_host?: string] {
  ...
  let pool_ids = (
    $acct_data | where { |d| $d.si.delegation != null } | each { |d| $d.si.delegation } | uniq
  )
  ...
  let delegated_lovelace = (
    $delegated_data | each { |d| $d.bal } | math sum | default 0
  )
...
def do-delegate [
  ...
  if $balance < $min_balance {
    error make --unspanned {
      msg: $"Insufficient balance: (lovelace-to-ada $balance). Need at least (lovelace-to-ada $min_balance)."
    }
  }
...
def do-defund [
  ...
  let acct_skey = (account-skey (root-key $mnemonic))
  let acct = (derive-account $acct_skey $index)
  let si = (try { query-stake-info $acct.stake_address $net } catch { {registered: false, delegation: null, rewards: 0} })
  let balance = (query-balance $acct.delegation_address $net)
...
def show-help [] {
  ...
  # Flags and options
  let f = (ansi yellow)
  # Descriptions / dim text
  let d = (ansi white_dimmed)
  # Paths
  let p = (ansi cyan_dimmed)
  # Step numbers
  let n = (ansi default)
  let r = (ansi reset)
  print $"
  ...
  match $resolved_cmd {
    ...
    "status" => {
      let n = if $num_accounts != null { $num_accounts } else {
        ...
        if $found == 0 {
          error make --unspanned {
            msg: $"No account directories found under (secrets-root $resolved_env)/. Run 'generate-accounts' first or pass -n."
          }
        }
        $found
      }
