{
  perSystem = {
    ...
    extraPkgs =
      config.devShells.dev.nativeBuildInputs
      ++ [
        config.packages.run-process-compose-dbsync-sanchonet
        config.packages.job-create-stake-pool-keys
      ];
    maxLayers = 100;
    nixConf = {};
    flake-registry = null;
    ...
    in
      pkgs.runCommand "base-system"
      {
        ...
        ''));
  in {
    packages.image = pkgs.dockerTools.buildLayeredImageWithNixDb {
      ...
      config = {
        ...
        Env = [
          ...
          "DATA_DIR=state-demo/rundir"

          "CARDANO_NODE_SOCKET_PATH=/home/node-demo.socket"
          "TESTNET_MAGIC=42"

          "NUM_GENESIS_KEYS=3"
          "POOL_NAMES=sp-1 sp-2 sp-3"
          "STAKE_POOL_DIR=state-demo/groups/stake-pools"

          "BULK_CREDS=state-demo/bulk.creds.all.json"
