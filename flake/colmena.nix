...
in
  ...
  with lib; {
    flake.colmena = let
      ...
      # full copy of it. Measured across the fleet on 2026-09-26, each snapshot
      # holds 2.0G to 3.6G, so the module default of 72 projects to 147G to
      # 259G of snapshots per host. 36 snapshots at the 20 minute cadence is a
      # 12h window and roughly half that.
      # The cadence stays at 20 minutes because leios-chain-snapshot cuts its
      # immutable chain from the newest autosnap.
      leiosSnapRetention.services.zfs-snapshots.keep = 36;
      ...
      # Ouroboros leios makes leios prototype packages available through its cardano-node-leios input
        ...
        // {
          ...
          systemd.services.cardano-node = {
            ...
            # host RAM in <1 day): recycle the service every ~12 hours so memory
            # can't grow unbounded. RuntimeMaxSec caps runtime;
            # RuntimeRandomizedExtraSec adds 0..N jitter so the fleet doesn't
            # restart in lockstep (thundering herd). Window 12-14h.
            ...
            serviceConfig = {
              ...
              RuntimeRandomizedExtraSec = 2 * 3600;
            };
          };
        };
      ...
      # Big ledger peer reachability census; one host per environment
      census = {imports = [inputs.cardano-parts.nixosModules.profile-cardano-census];};
      ...
      faucetReleasePin = moduleWithSystem ({system}: _: {
        imports = [
          {cardano-parts.perNode.pkgs.cardano-faucet = withSystem system ({config, ...}: mkForce config.cardano-parts.pkgs.cardano-faucet);}
        ];
      });
    ...
    in {
      ...
      preprod1-bp-a-1 = {imports = [eu-central-1 r6a-large (ebs 80) (nodeRamPct 70) (group "preprod1") node bp mithrilRelease mithrilSignerStack (declMRel "preprod1-rel-a-1") ccMon];};
      ...
      preprod1-dbsync-a-1 = {imports = [eu-central-1 r6a-xlarge (ebs 200) (group "preprod1") dbsync smash preprodSmash census];};
      preprod1-faucet-a-1 = {imports = [eu-central-1 r6a-large (ebs 80) (nodeRamPct 70) (group "preprod1") node faucet faucetReleasePin preprodFaucet];};

      preprod2-bp-b-1 = {imports = [eu-west-1 r6a-large (ebs 80) (nodeRamPct 70) (group "preprod2") node bp mithrilRelease mithrilSignerStack (declMRel "preprod2-rel-b-1")];};
      ...
      preprod3-bp-c-1 = {imports = [us-east-2 r6a-large (ebs 80) (nodeRamPct 70) (group "preprod3") node-pre bp traceMp mithrilRelease mithrilSignerStack (declMRel "preprod3-rel-c-1")];};
      ...
      preview1-bp-a-1 = {imports = [eu-central-1 r6a-large (ebs 80) (nodeRamPct 70) (group "preview1") node bp mithrilRelease (declMRel "preview1-rel-a-1") ccMon];};
      ...
      preview1-dbsync-a-1 = {imports = [eu-central-1 r6a-large (ebs 250) (group "preview1") dbsync smash previewSmash census];};
      preview1-faucet-a-1 = {imports = [eu-central-1 r6a-large (ebs 80) (nodeRamPct 70) (group "preview1") node faucet faucetReleasePin previewFaucet];};
      ...
      dijkstra1-bp-a-1 = {imports = [eu-central-1 t3a-medium (ebs 80) (group "dijkstra1") node bp ccMon];};
      dijkstra1-rel-a-1 = {imports = [eu-central-1 t3a-medium (ebs 80) (group "dijkstra1") node rel];};
      dijkstra1-dbsync-a-1 = {imports = [eu-central-1 t3a-medium (ebs 250) (group "dijkstra1") dbsync smash dijkstraSmash census];};
      dijkstra1-faucet-a-1 = {imports = [eu-central-1 t3a-medium (ebs 80) (group "dijkstra1") node faucet faucetReleasePin dijkstraFaucet];};
      ...
      # Regions for which only older generation machines with ephemeral NVMe are available are sized up to avoid compute deficiency.
      leiosred1-bp-a-1 = {imports = [eu-central-1 c8id-xlarge (ebs 300) (group "leiosred1") node-leios leiosRedTeamBp];};
      leiosred2-bp-b-1 = {imports = [eu-west-1 c6id-2xlarge (ebs 300) (group "leiosred2") node-leios leiosRedTeamBp];};
      leiosred3-bp-c-1 = {imports = [us-east-2 c8id-xlarge (ebs 300) (group "leiosred3") node-leios leiosRedTeamBp];};
      leiosred4-bp-d-1 = {imports = [eu-north-1 c8id-xlarge (ebs 300) (group "leiosred4") node-leios leiosRedTeamBp];};
      leiosred5-bp-e-1 = {imports = [ap-southeast-2 c6id-2xlarge (ebs 300) (group "leiosred5") node-leios leiosRedTeamBp];};
      leiosred6-bp-f-1 = {imports = [sa-east-1 c6id-2xlarge (ebs 300) (group "leiosred6") node-leios leiosRedTeamBp];};
      leiosred7-bp-g-1 = {imports = [af-south-1 c5d-2xlarge (ebs 300) (group "leiosred7") node-leios leiosRedTeamBp];};
      leiosred8-bp-h-1 = {imports = [ap-northeast-1 c8id-xlarge (ebs 300) (group "leiosred8") node-leios leiosRedTeamBp];};
      leiosred9-bp-i-1 = {imports = [us-west-1 c5d-2xlarge (ebs 300) (group "leiosred9") node-leios leiosRedTeamBp];};
      leiosred10-bp-j-1 = {imports = [us-west-2 c8id-xlarge (ebs 300) (group "leiosred10") node-leios leiosRedTeamBp];};
      # ---------------------------------------------------------------------------------------------------------
      #
      # ---------------------------------------------------------------------------------------------------------
      # Mainnet
      ...
      mainnet1-dbsync-a-1 = {imports = [eu-central-1 r5-2xlarge (ebs 1000) (group "mainnet1") dbsync smash mainnetSmash dbsyncPub (openFwTcp 5432) traceMp census {services.cardano-db-sync.nodeRamAvailableMiB = 20480;}];};
      ...
      sanchonet1-dbsync-a-1 = {imports = [eu-central-1 r6a-xlarge (ebs 250) (group "sanchonet1") dbsync-pre smash sanchonetSmash ccMon census];};
